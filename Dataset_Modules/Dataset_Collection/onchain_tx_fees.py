"""
col_onchain_tx_fees.py
-----------------------
Single-column dataset feature.

Column : Onchain Transaction Fees (BTC)
Source : Blockchain.info API – charts/transaction-fees
         (total value of all transaction fees paid to miners per day, in BTC)
Output : Dataset_Modules/dataset_output/2015-2025_onchain_tx_fees.csv

Run standalone:
    python col_onchain_tx_fees.py
...
OUTPUT_FILENAME = "onchain_tx_fees.csv"
COLUMN_NAME     = "Onchain Transaction Fees (BTC)"
ENDPOINT        = "charts/transaction-fees"


def collect(start_date: str, end_date: str, date_range: pd.DatetimeIndex, freq: str = "1d") -> pd.DataFrame:
    """Fetch the daily total Bitcoin transaction fees from the Blockchain.info API.
...
def main() -> None:
    """Parse CLI arguments, collect transaction fees data, and save the output CSV."""
    import argparse
