...
def collect(start_date: str, end_date: str, date_range: pd.DatetimeIndex, interval: str = "1d") -> pd.DataFrame:  # (Anthropic, 2026)
    ...
    Downloads each index defined in GLOBAL_INDICES, with closing prices
    converted to USD, and averages the per-index USD close values across
    all indices that have data on each day.  If no price columns are
    ...
    Returns:
        ...
        "Global averaged stocks(USD)" containing the mean USD closing
        price across all available global indices for each date.
    """
